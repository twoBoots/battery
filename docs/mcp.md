...
| `battery_list_barrels` | Lists registered barrels and resolves their [Cooper](https://twoboots.github.io/cooper) tech stacks (`.cooper/definition/tech-stack.md`). | *None* |
...
| `battery_dispatch_track` | Dispatches spec deltas to barrel worktrees managed by [Troop](https://twoboots.github.io/troop) while omitting `plan.md` to preserve local planning autonomy. | `track_id` (string, required), `force` (boolean) |
